...
export class SeriesDetailComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.utilService.isEmpty(this.config.data?.id) == false) {
      ...
      // Create form for new entity only after data is ready
      this.buildForm();
    ...
    this.seriesApiClient
      ...
      .subscribe({
        next: (response: SeriesDto) => {
          this.selectedEntity = response as SeriesDto;
          this.buildForm();
          this.toggleBlockUI(false);
        },
        error: () => {
  ...
  buildForm() {
    ...
    // Ensure Angular updates view after building form to avoid ExpressionChangedAfterItHasBeenCheckedError
