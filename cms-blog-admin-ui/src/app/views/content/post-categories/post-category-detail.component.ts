...
export class PostCategoryDetailComponent implements OnInit, OnDestroy {
    ...
    public generateSlug() {
        ...
        const slug = this.utilService.makeSeoTitle(name);
    ...
    private saveData() {
        // Check for duplicate name before creating/updating
        this.postCategoryService.getPostCategories()
            .pipe(takeUntil(this.ngUnsubscribe))
            .subscribe({
                next: (categories: PostCategoryDto[]) => {
                    const inputName = (this.form.get('name')?.value || '').toString().trim().toLowerCase();
                    const duplicate = (categories || []).some(cat => {
                        const catName = (cat.name || '').toString().trim().toLowerCase();
                        // if editing, ignore the same record
                        const isSameRecord = this.config.data?.id && cat.id === this.config.data.id;
                        return !isSameRecord && catName === inputName;
                    });

                    if (duplicate) {
                        this.alertService.showError('Tên danh mục đã tồn tại');
                        this.toggleBlockUI(false);
                        return;
                    }
                ...
                error: () => {
                    // if failed to fetch categories, still attempt save (or show error)
