...
export class PermissionGrantComponent implements OnInit, OnDestroy {
        ...
        this.roleService
            ...
            .subscribe({
                next: (response: PermissionDto) => {
                    // Ensure permissions is always an array (API may return undefined)
                    this.permissions = response.roleClaims ?? [];
                    // reset selected permissions before rebuilding the form
    ...
    buildForm() {
        ...
        // Reset selected permissions and fill values
        this.selectedPermissions = [];
        for (let index = 0; index < (this.permissions ?? []).length; index++) {
